...
<body>
    ...
    <?php
            if(isset($_GET['reponse'])){
                echo "<p style='background-color:red; color: white;'>{$_GET['reponse']}</p>";
            }
        ?>
        <a href="add_author.php"><button class="btn btn-success" type = "submit">Thêm mới</button></a>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
            ...
            foreach($data as $value): ?>
                ...
                <td><?= $value['hinh_tgia']; ?></td>
                <td><a href= "edit_author.php?id=<?= $value['ma_tgia'];?>"><i class="bi bi-pencil-square"></a></i></td>
                <td><a href= "delete_author.php?id=<?= $value['ma_tgia'];?>" onclick="return confirm('Bạn có chắc chắn muốn xoá?');"><i class="bi bi-trash3-fill"></a></i></td>
                </tr>
                <?php endforeach;?>
            </tbody>
        </table>
