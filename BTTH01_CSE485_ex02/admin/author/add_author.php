...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Truy vấn kiểm tra xem tên tác giả đã tồn tại hay chưa
        $sql_check = "SELECT COUNT(*) FROM tacgia WHERE ten_tgia = '$ten_tgia'";
        ...
        if ($rowCount > 0) {
            $reponse = "Tên tác giả đã tồn tại";
            header("Location: author.php?reponse=" . urlencode($reponse));
            exit(); // Dừng thực hiện script sau khi chuyển hướng
        } else {
            // Thêm tác giả mới vào cơ sở dữ liệu
            $sql_insert = "INSERT INTO tacgia (ten_tgia, hinh_tgia) VALUES (:ten_tgia, :hinh_tgia)";
            $stmt_insert = $conn->prepare($sql_insert);
            $stmt_insert->bindParam(":ten_tgia", $ten_tgia, PDO::PARAM_STR);
            $stmt_insert->bindParam(":hinh_tgia", $hinh_tgia, PDO::PARAM_STR);
            $stmt_insert->execute();

            $rowCount = $stmt_insert->rowCount();

            if ($rowCount > 0) {
                $reponse = "Đã thêm thành công tác giả mới";
                header("Location: author.php?reponse=" . urlencode($reponse));
                exit();
            }
        }
...
<body>
    ...
    <div class = "Edit">
        <center><h2>Thêm mới tác giả</h2></center>
        <form action="add_author.php" method="POST">
            ...
            <div class="input-group mb-3">
                ...
                <input type="text" class="form-control" name = "hinh_tgia" required>
            </div>
